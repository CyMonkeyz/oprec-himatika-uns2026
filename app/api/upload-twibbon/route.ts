// app/api/upload-twibbon/route.ts
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const folderId = process.env.GOOGLE_DRIVE_TWIBBON_UPLOAD_FOLDER_ID;
    if (!folderId) {
      return NextResponse.json({ ok: false, message: "Folder Twibbon belum dikonfigurasi." }, { status: 500 });
    }
    ...
    const name = `${baseName} - Twibbon.${ext}`;
